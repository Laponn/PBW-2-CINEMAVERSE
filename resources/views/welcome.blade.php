...
<body class="bg-[#050509] text-white font-sans">
    ...
    <header class="fixed top-0 left-0 right-0 z-40 bg-black/80 backdrop-blur border-b border-red-900/40 h-24 transition-all duration-300">
        ...
        <div class="max-w-screen-2xl mx-auto px-6 h-full flex items-center justify-between">
            ...
            {{-- MENU TENGAH --}}
            <nav class="hidden md:flex items-center gap-10 text-base font-medium"> {{-- text-sm jadi text-base, gap-8 jadi gap-10 --}}
                ...
                @if (Route::has('login'))
                    <div class="flex items-center gap-4">
                        @auth
                            ...
                            {{-- Tombol Dashboard --}}
                            <a href="{{ url('/dashboard') }}" class="text-sm font-semibold border border-zinc-600 rounded-full px-5 py-2 hover:border-red-500 hover:text-red-400 transition">
                                Dashboard
                            </a>
                            ...
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit" class="text-sm font-bold bg-red-600 text-white rounded-full px-5 py-2 hover:bg-red-500 transition shadow-lg shadow-red-900/50 hover:shadow-red-600/50">
                                    Log Out
                                </button>
                            </form>
                        ...
                        @else
                            {{-- Jika Belum Login --}}
                            <a href="{{ route('login') }}" class="text-sm font-bold text-zinc-300 hover:text-white transition">
                                Log in
                            </a>
                            @if (Route::has('register'))
                                <a href="{{ route('register') }}" class="hidden sm:inline-flex text-sm font-bold bg-white text-black rounded-full px-5 py-2 hover:bg-gray-200 transition transform hover:scale-105">
                                    Register
                                </a>
                            @endif
                        @endauth
            ...
            <section id="now-showing">
                <div class="flex items-end justify-between mb-10 border-b border-zinc-800 pb-4">
                    ...
                    <div class="hidden md:block">
                         <a href="#" class="text-sm font-semibold text-red-500 hover:text-red-400">View All Movies &rarr;</a>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-8">
                        ...
                        <div class="group bg-zinc-900 rounded-2xl overflow-hidden border border-zinc-800 hover:border-red-600/50 hover:shadow-[0_0_30px_rgba(220,38,38,0.15)] transition duration-500 relative">
                            ...
                            <div class="relative aspect-[2/3] overflow-hidden">
                                ...
                                <div class="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition duration-300">
                                    <a href="{{ route('movies.show', $movie->id) }}" class="bg-red-600 text-white rounded-full p-4 shadow-lg transform scale-0 group-hover:scale-100 transition duration-300">
                    ...
                    @empty
                        <div class="col-span-full flex flex-col items-center justify-center py-20 text-zinc-500">
                            <svg class="w-16 h-16 mb-4 opacity-30" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 4v16M17 4v16M3 8h4m10 0h4M3 12h18M3 16h4m10 0h4M4 20h16a1 1 0 001-1V5a1 1 0 00-1-1H4a1 1 0 00-1 1v14a1 1 0 001 1z"></path></svg>
