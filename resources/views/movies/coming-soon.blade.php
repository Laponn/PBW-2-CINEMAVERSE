...
<main class="pt-24 pb-16 min-h-screen bg-[#050509]">
    <div class="max-w-screen-2xl mx-auto px-6 space-y-12">
        ...
        {{-- BACK ICON (STYLE SAMA KAYAK MOVIES/SHOW.BLADE.PHP) --}}
        <a href="{{ route('home') }}"
           class="inline-flex items-center gap-3 text-[10px] font-black text-zinc-400 hover:text-red-500 transition group uppercase tracking-[0.2em]">
        ...
        {{-- HEADER RAPiH (KIRI JUDUL, KANAN TAB+SEARCH) --}}
        <section class="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
            ...
            <div class="space-y-4">
                ...
                <h1 class="text-5xl md:text-7xl font-black leading-[0.9] tracking-tighter italic uppercase text-white">
                    Film <span class="text-red-600">Akan</span><br class="hidden md:block"/>
                    <span class="text-red-600">Tayang</span>
                </h1>

                <p class="text-zinc-500 font-bold uppercase tracking-[0.2em] text-[10px]">
                    Cek film yang segera hadir & tonton trailer
                </p>
            </div>
            ...
            <div class="space-y-4 lg:justify-self-end w-full lg:w-[560px]">
                ...
                <div class="flex items-center justify-end">
                    <div class="inline-flex items-center p-2 rounded-full bg-white/5 border border-white/10">
                        {{-- Now Showing --}}
                        <a href="{{ route('home') }}"
                           class="px-6 py-3 rounded-full text-zinc-200 text-[10px] font-black uppercase tracking-[0.25em]
                        ...
                        {{-- Active: Coming Soon --}}
                        <span
                            class="ml-2 px-6 py-3 rounded-full bg-red-600 text-white text-[10px] font-black uppercase tracking-[0.25em]
                    ...
                    <div class="flex items-center gap-3">
                        <div class="relative flex-1">
                            <input type="text" name="q" value="{{ request('q') }}"
                                   ...
                                   placeholder="Cari film coming soon...">
            ...
            <section class="pt-2">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                    @foreach($movies as $movie)
                        <div class="group bg-zinc-900/30 border border-white/5 rounded-[2.5rem] overflow-hidden backdrop-blur-sm hover:border-red-600/30 transition shadow-2xl">
                            ...
                            <div class="p-6 space-y-4">
                                ...
                                <p class="text-zinc-400 text-sm leading-relaxed font-medium italic">
                                    "{{ \Illuminate\Support\Str::limit($movie->description, 120) }}"
                                </p>

                                <div class="flex flex-wrap gap-3 pt-2">
                                    @if($movie->trailer_url)
                                        <a href="{{ $movie->trailer_url }}" target="_blank"
                                           class="inline-flex items-center gap-2 px-5 py-3 rounded-full bg-white/5 border border-white/10 hover:bg-white/10 text-white text-[11px] font-black uppercase tracking-[0.2em] transition">
                                            ▶ Trailer
                                        </a>
                                    @else
                                        <span class="inline-flex items-center gap-2 px-5 py-3 rounded-full bg-white/5 border border-white/10 text-zinc-500 text-[11px] font-black uppercase tracking-[0.2em]">
                                            Trailer -
                                        </span>
                                    @endif
                                </div>
